...
void mp3Stop(void)              { usartSendByte('O');        }  // to stop
...
{
	    usartSendByte('Q');          /* ask status  */
	    _delay_ms(3);                /* Trigger responds within 1?2 ms */
	
	    if(!(UCSR0A & (1 << RXC0)))  /* no byte? treat as busy?false */
	        return 0;
	
	    uint8_t resp = UDR0;         /* 0 = idle, 1 = playing       */
	    return resp == 1;
}